...
tickers = st.sidebar.text_input("Tickery (oddzielone przecinkami)", value="CDR.WA")
st.sidebar.caption("Używaj tickerów w formacie Yahoo Finance, np. CDR.WA, AAPL, ^GSPC.")
...
n_simulations = st.sidebar.number_input("Liczba ścieżek MC", min_value=100, max_value=10_000, value=1000, step=100)
...
selected_freq = interval_to_freq[yahoo_interval]
...
if st.sidebar.button("Uruchom analizę"):
    ...
    for ticker, asset in assets.items():
        ...
        st.dataframe(asset.prices.sort_index(ascending=False).head(10))
